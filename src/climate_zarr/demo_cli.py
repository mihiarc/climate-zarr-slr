...
def show_interactive_features():
    ...
    features_table.add_row(
        "🧙‍♂️ Interactive Wizard", "Complete guided experience", "climate-zarr wizard"
    )
    features_table.add_row(
        "🗜️ Smart Conversion",
...
def main():
    ...
    if not cli_path.exists():
        console.print(
            "[red]❌ CLI script not found. Please run from the project directory.[/red]"
        )
        return

    # Show interactive features overview
    print_section_header(
        "🌟 Interactive Features Overview", "Modern CLI patterns with beautiful UX"
    )
    show_interactive_features()

    # Demo 1: Help and Information Commands
    print_section_header(
        "📖 Demo 1: Help & Information", "Discover what the CLI can do"
    )
    ...
    print_section_header(
        "🛡️ Demo 5: Smart Error Handling", "Intelligent error recovery and suggestions"
    )
    ...
    # Success summary
    print_section_header(
        "🎉 Demo Complete!", "You've seen the modern interactive CLI in action"
    )
