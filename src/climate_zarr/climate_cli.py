...
def interactive_variable_selection() -> str:
    ...
    choices = [
        questionary.Choice(
            title="🌟 All Variables - process all climate variables", value="all"
        )
    ] + [
...
def interactive_file_selection() -> Path:
    ...
    if suggested_paths:
        ...
        for path, count in suggested_paths:
            choices.append(
                questionary.Choice(
                    title=f"📁 {path.name}/ ({count} NetCDF files)", value=str(path)
                )
            )
...
def validate_region(region: str) -> str:
    ...
    if region.lower() not in available_regions:
        ...
        if questionary.confirm(
            "🤔 Would you like to select from available regions?"
        ).ask():
...
def create_zarr(
    input_path: Annotated[
        Optional[Path],
        typer.Argument(help="Directory containing NetCDF files or single NetCDF file"),
    ] = None,
    output: Annotated[
        Optional[Path], typer.Option("--output", "-o", help="Output Zarr store path")
    ] = None,
    region: Annotated[
        Optional[str],
        typer.Option("--region", "-r", help="Clip data to specific region"),
    ] = None,
    concat_dim: Annotated[
        str, typer.Option("--concat-dim", "-d", help="Dimension to concatenate along")
    ] = "time",
    chunks: Annotated[
        Optional[str],
        typer.Option("--chunks", "-c", help="Chunk sizes as 'dim1=size1,dim2=size2'"),
    ] = None,
    compression: Annotated[
        str, typer.Option("--compression", help="Compression algorithm")
    ] = "default",
    compression_level: Annotated[
        int, typer.Option("--compression-level", help="Compression level (1-9)")
    ] = 5,
    ...
    if not output and interactive:
        suggested = Path(
            f"{input_path.stem}_climate.zarr"
            if input_path.is_file()
            else "climate_data.zarr"
        )
...
def county_stats(
    zarr_path: Annotated[
        Optional[Path], typer.Argument(help="Path to Zarr dataset")
    ] = None,
    region: Annotated[
        Optional[str], typer.Argument(help="Region name (conus, alaska, hawaii, etc.)")
    ] = None,
    output: Annotated[
        Optional[Path], typer.Option("--output", "-o", help="Output CSV file")
    ] = None,
    variable: Annotated[
        Optional[str],
        typer.Option("--variable", "-v", help="Climate variable to analyze"),
    ] = None,
    scenario: Annotated[
        str, typer.Option("--scenario", "-s", help="Scenario name")
    ] = "historical",
    threshold: Annotated[
        Optional[float], typer.Option("--threshold", "-t", help="Threshold value")
    ] = None,
    workers: Annotated[
        int, typer.Option("--workers", "-w", help="Number of worker processes")
    ] = 4,
    use_distributed: Annotated[
        bool, typer.Option("--distributed", help="Use Dask distributed processing")
    ] = False,
    interactive: Annotated[
        bool,
        typer.Option(
    ...
    if threshold is None and interactive:
        default_threshold = (
            "25.4" if variable == "pr" else "32" if variable == "tasmax" else "0"
        )
    ...
    if not output and interactive:
        ...
        suggested_path = output_manager.get_output_path(
            variable=variable, region=region, scenario=scenario, threshold=threshold
        )
        output = Path(Prompt.ask("📊 Output CSV file", default=str(suggested_path)))
    elif not output:
    ...
    if interactive and workers > 8:
        if not Confirm.ask(
            f"⚠️ Use {workers} workers? This will use significant system resources."
        ):
            workers = 4
    ...
    try:
        # Create processor
        processor = ModernCountyProcessor(n_workers=workers)
        ...
        # Process data
        console.print(
            f"[blue]🔄 Processing {variable.upper()} data for {len(gdf)} counties...[/blue]"
        )
        ...
        output_manager.save_with_metadata(
            data=results_df, output_path=output, metadata=metadata, save_method="csv"
        )
        ...
        summary_table.add_row(
            "Counties Processed", str(len(results_df["county_id"].unique()))
        )
...
def interactive_wizard():
    ...
    if operation in ["convert", "pipeline"]:
        ...
        if variable_selection == "all":
            ...
            console.print(
                f"[cyan]📊 Will convert all variables: {', '.join(variables_to_process)}[/cyan]"
            )
        else:
        ...
        for variable in variables_to_process:
            ...
            for data_path in possible_paths:
                if data_path.exists():
                    ...
                    if pattern_files and variable in str(pattern_files[0]).lower():
                        found_files.extend(
                            [f for f in pattern_files if variable in str(f).lower()]
                        )
                        break
            ...
            if found_files:
                ...
                console.print(
                    f"[green]✅ Found {len(found_files)} files for {variable.upper()}: {found_files[0].parent}[/green]"
                )
            else:
                missing_vars.append(variable)
                console.print(
                    f"[yellow]⚠️ No files found for {variable.upper()}[/yellow]"
                )
        ...
        if missing_vars:
            ...
            if manual_path.exists():
                ...
                if manual_files:
                    ...
                    console.print(
                        f"[green]✅ Added {len(manual_files)} manual files[/green]"
                    )

        if not nc_files:
            console.print(
                "[red]❌ No NetCDF files found. Please check your data directory structure.[/red]"
            )
            console.print(
                "[dim]Expected structure: data/{variable}/historical/{variable}_*.nc[/dim]"
            )
            return
        ...
        if missing_vars:
            console.print(
                f"[yellow]📝 Manual selection: {', '.join(missing_vars).upper()}[/yellow]"
            )
        console.print(
            f"[bold green]✅ Total: {len(nc_files)} NetCDF files ready for conversion[/bold green]"
        )

        # Use recommended compression by default
        compression = "zstd"
        console.print(
            "[green]🗜️ Using ZSTD compression (recommended - fast & efficient)[/green]"
        )
        ...
        try:
            ...
            for variable, variable_files in files_by_variable.items():
                ...
                console.print(
                    f"[green]✅ {variable.upper()} complete: {zarr_path}[/green]"
                )
    ...
    if operation in ["stats", "pipeline"]:
        ...
        if operation == "stats":
            ...
            if variable_selection == "all":
                variables_to_process = all_variables
                console.print(
                    f"[cyan]📊 Will process all variables: {', '.join(variables_to_process)}[/cyan]"
                )
            else:
                variables_to_process = [variable_selection]
        else:
            ...
            console.print(
                f"[cyan]📊 Processing statistics for: {', '.join(variables_to_process)}[/cyan]"
            )
        ...
        try:
            ...
            # Create processor
            processor = ModernCountyProcessor(n_workers=int(workers))
            ...
            for variable in variables_to_process:
                ...
                if operation == "stats":
                    ...
                    expected_zarr = Path(
                        f"climate_outputs/zarr/{variable}/{stats_region}/historical/{stats_region}_historical_{variable}.zarr"
                    )
                    if expected_zarr.exists():
                        current_zarr_path = expected_zarr
                        console.print(
                            f"[green]✅ Found Zarr: {current_zarr_path}[/green]"
                        )
                    else:
                        console.print(
                            f"[yellow]⚠️ Skipping {variable}: Zarr not found at {expected_zarr}[/yellow]"
                        )
                        continue
                else:
                    ...
                    for created_path in created_zarr_paths:
                        if f"/{variable}/" in str(
                            created_path
                        ) and f"_{variable}.zarr" in str(created_path):
                    ...
                    if variable_zarr:
                        ...
                        console.print(
                            f"[yellow]⚠️ Skipping {variable}: Could not find corresponding zarr file[/yellow]"
                        )
                        continue
                ...
                console.print(
                    f"[green]✅ {variable.upper()} complete: {full_csv_path}[/green]"
                )
...
def list_regions():
    ...
    for region_key, region_config in CONFIG.regions.items():
        ...
        regions_table.add_row(region_key, region_config.name, bounds)
...
def info():
    ...
    # System info
    info_layout = Layout()
    info_layout.split_column(Layout(name="data"), Layout(name="regions"))
