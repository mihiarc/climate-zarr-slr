...
class ModernCountyProcessor:
    ...
    def process_zarr_data(
        ...
        Args:
            ...
            chunk_by_county: Backward compatibility parameter (deprecated, ignored)
        ...
        if variable not in self._processors:
            raise ValueError(
                f"Unsupported variable: {variable}. Supported: {list(self._processors.keys())}"
            )
        ...
        if variable not in ds.data_vars:
            raise ValueError(
                f"Variable '{variable}' not found in dataset. Available: {list(ds.data_vars)}"
            )
        ...
        if variable == "pr":
            return processor.process_variable_data(
                data=data, gdf=gdf, scenario=scenario, threshold_mm=threshold
            )
        elif variable == "tas":
            return processor.process_variable_data(
                data=data, gdf=gdf, scenario=scenario
            )
        elif variable == "tasmax":
            return processor.process_variable_data(
                data=data, gdf=gdf, scenario=scenario, threshold_temp_c=threshold
            )
