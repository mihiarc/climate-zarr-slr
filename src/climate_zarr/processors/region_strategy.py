...
def get_strategy_for_region(
    region: str, gdf: gpd.GeoDataFrame, n_workers: int = 4
) -> Any:
    ...
    if region.lower() == "conus":
        console.print(
            f"[green]Using SpatialChunkedStrategy for CONUS ({num_counties} counties)[/green]"
        )
        console.print(
            "[cyan]Benefits: Parallel processing, spatial optimization, memory efficiency[/cyan]"
        )
        ...
        console.print(
            f"[green]Using VectorizedStrategy for {region.upper()} ({num_counties} counties)[/green]"
        )
        console.print(
            "[cyan]Benefits: Simple processing, predictable memory, precise geometry handling[/cyan]"
        )
