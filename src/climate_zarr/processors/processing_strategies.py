...
from ..utils.spatial_utils import get_time_information, clip_county_data
...
class SpatialChunkedStrategy(ProcessingStrategy):
    ...
    def process(
        ...
        console.print(
            "[yellow]Initializing spatial chunked processing strategy...[/yellow]"
        )
        ...
        console.print(
            f"[cyan]Target memory usage: {target_memory_gb:.1f} GB ({self.target_memory_usage * 100:.0f}%)[/cyan]"
        )
        ...
        console.print(
            f"[green]Created {len(chunks)} spatial chunks for processing[/green]"
        )
        ...
        with ThreadPoolExecutor(max_workers=min(n_workers, len(chunks))) as executor:
            ...
            with Progress(
                ...
                task = progress.add_task(
                    f"Processing {len(chunks)} chunks...", total=len(chunks)
                )
                ...
                for future in as_completed(future_to_chunk):
                    ...
                    try:
                        ...
                        if current_memory > 85:
                            console.print(
                                f"[yellow]Warning: High memory usage ({current_memory:.1f}%)[/yellow]"
                            )
        ...
        if failed_chunks:
            console.print(
                f"[yellow]Retrying {len(failed_chunks)} failed chunks sequentially...[/yellow]"
            )
            for chunk_id in failed_chunks:
                try:
                    ...
                    console.print(
                        f"[red]Chunk {chunk_id} failed on retry: {str(e)}[/red]"
                    )

        console.print(
            f"[green]Spatial chunked processing complete: {len(results)} county-years processed[/green]"
        )
    ...
    def _create_spatial_chunks(
        ...
        console.print(
            f"[cyan]Creating {n_initial_clusters} initial spatial clusters (memory: {memory_based_clusters}, workers: {worker_based_clusters})[/cyan]"
        )
    ...
    def _estimate_county_memory_usage(
        self, data: xr.DataArray, gdf: gpd.GeoDataFrame
    ) -> np.ndarray:
        ...
        for _, county in gdf.iterrows():
            ...
            boundary_complexity = (
                len(county.geometry.exterior.coords)
    ...
    def _refine_clusters_by_memory(
        self,
        ...
        for cluster_id in unique_clusters:
            ...
            if (
                np.sum(cluster_memory) > target_memory_gb * 0.8
            ):  # 80% of target per chunk
                ...
                for idx in sorted_indices:
                    if (
                        current_memory + memory_estimates[idx] < target_memory_gb * 0.8
                        and len(current_chunk) < self.max_chunk_size
                    ):
                ...
                if len(cluster_indices) >= self.min_chunk_size:
                    ...
                    if (
                        chunks
                        and len(chunks[-1]) + len(cluster_indices)
                        <= self.max_chunk_size
                    ):
    ...
    def _optimize_spatial_locality(
        self, chunks: List[List[int]], gdf: gpd.GeoDataFrame
    ) -> List[List[int]]:
        ...
        for chunk in chunks:
            ...
            if chunk_centroids:
                ...
                while remaining:
                    distances = [
                        current_centroid.distance(gdf.iloc[i].geometry.centroid)
                        for i in remaining
                    ]
    ...
    def _process_chunk(
        ...
        try:
            ...
            console.print(
                f"[cyan]Processing chunk {chunk_id} with {len(county_indices)} counties[/cyan]"
            )
            ...
            for county_idx, county in zip(county_indices, chunk_counties):
                try:
                    ...
                    console.print(
                        f"[red]Error processing county {county_idx} in chunk {chunk_id}: {str(e)}[/red]"
                    )
                    continue
            ...
            console.print(
                f"[green]Chunk {chunk_id} complete: {len(chunk_results)} results[/green]"
            )
    ...
    def _process_county_years_chunked(
        self,
            ...
            try:
                for year in year_chunk:
                    ...
                    if year_data.size > 100000:  # For large datasets, use Dask
                        daily_means = (
                            year_data.mean(dim=["y", "x"], skipna=True).compute().values
                        )
                    else:
                ...
                console.print(
                    f"[red]Error processing years {year_chunk[0]}-{year_chunk[-1]} for {county_info['county_name']}: {str(e)}[/red]"
                )
                continue
    ...
    def _process_chunk_fallback(
        ...
        console.print(
            f"[yellow]Using fallback sequential processing for {len(county_indices)} counties[/yellow]"
        )
        ...
        for county_idx in county_indices:
            try:
                ...
                console.print(
                    f"[red]Fallback failed for county {county_idx}: {str(e)}[/red]"
                )
                continue

        return results

    def _process_county_years_basic(
        self,
        clipped_data: xr.DataArray,
        county,
        years: np.ndarray,
        ...
        for year in unique_years:
            try:
                ...
                console.print(
                    f"[red]Error processing year {year} for {county_info['county_name']}: {str(e)}[/red]"
                )
                continue
    ...
    def _validate_spatial_data(self, data: xr.DataArray, gdf: gpd.GeoDataFrame) -> None:
        ...
        if data.rio.crs is None:
            console.print(
                "[yellow]Warning: Data CRS not specified, assuming EPSG:4326 (WGS84)[/yellow]"
            )
        ...
        if gdf.crs is None:
            console.print(
                "[yellow]Warning: County GeoDataFrame CRS not specified, assuming EPSG:4326 (WGS84)[/yellow]"
            )
        ...
        if str(data.rio.crs) != str(gdf.crs):
            console.print(
                f"[yellow]CRS mismatch detected - Data: {data.rio.crs}, Counties: {gdf.crs}[/yellow]"
            )
    ...
    def _clip_county_optimized(self, data: xr.DataArray, county) -> xr.DataArray:
        ...
        try:
            ...
            console.print(
                f"[red]Clipping failed for {county.get('county_name', 'Unknown')}: {str(e)}[/red]"
            )
            return None
...
class VectorizedStrategy(ProcessingStrategy):
    ...
    def process(
        ...
        console.print(
            "[yellow]Processing counties with optimized rioxarray clipping...[/yellow]"
        )
        ...
        console.print(
            f"[cyan]Processing {len(gdf)} counties over {len(unique_years)} years[/cyan]"
        )
            ...
            for idx, county in gdf.iterrows():
                try:
                    ...
                    if clipped is not None and clipped.size > 0:
                        ...
                        empty_clips += 1
                        console.print(
                            f"[yellow]Warning: No data found for {county['county_name']}, {county['state']}[/yellow]"
                        )

                except Exception as e:
                    failed_counties += 1
                    console.print(
                        f"[red]Error processing {county['county_name']}, {county['state']}: {str(e)}[/red]"
                    )
        ...
        console.print(
            f"[green]Processing complete: {successful_counties} successful, {empty_clips} empty clips, {failed_counties} failed[/green]"
        )
