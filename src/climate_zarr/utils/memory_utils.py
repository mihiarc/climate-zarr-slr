...
def estimate_chunk_memory_usage(
    ...
    estimated_gb = (
        avg_county_memory_gb
        * counties_in_chunk
        * boundary_complexity_factor
        * processing_overhead
    )
...
def calculate_optimal_chunk_size(
    ...
    console.print(
        f"[cyan]Estimated memory per chunk: {estimate_chunk_memory_usage(data_shape, 8, optimal_size):.2f} GB[/cyan]"
    )
...
def adaptive_chunk_sizing(
    ...
    if memory_status == "critical":
        ...
        console.print(
            f"[red]Critical memory pressure: reducing chunk size to {adjusted}[/red]"
        )
        return adjusted
        ...
        console.print(
            f"[yellow]Memory warning: reducing chunk size to {adjusted}[/yellow]"
        )
        return adjusted
        ...
        if (
            processing_time_per_chunk and processing_time_per_chunk < 30
        ):  # Less than 30 seconds
            adjusted = min(50, int(base_chunk_size * 1.2))
            if adjusted > base_chunk_size:
                console.print(
                    f"[green]Fast processing: increasing chunk size to {adjusted}[/green]"
                )
            return adjusted
...
class ChunkPerformanceTracker:
    ...
    def record_chunk_performance(
        self, chunk_size: int, processing_time: float, peak_memory_percent: float
    ):
    ...
    def get_performance_stats(self) -> Dict[str, float]:
        ...
        return {
            "avg_time_per_county": np.mean(
                [t / s for t, s in zip(self.chunk_times, self.chunk_sizes)]
            ),
            "avg_chunk_time": np.mean(self.chunk_times),
            "avg_memory_usage": np.mean(self.memory_usage),
            "total_chunks": len(self.chunk_times),
            "fastest_time_per_county": min(
                [t / s for t, s in zip(self.chunk_times, self.chunk_sizes)]
            ),
    ...
    def recommend_optimal_chunk_size(self) -> Optional[int]:
        ...
        acceptable_indices = [
            i for i, mem in enumerate(self.memory_usage) if mem < 85.0
        ]
