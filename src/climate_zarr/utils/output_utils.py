...
class OutputManager:
    ...
    def create_summary_report(
        ...
        summary_data: Dict,
        report_name: str = "processing_summary",
    ) -> Path:
    ...
    def clean_old_outputs(self, days_old: int = 30, dry_run: bool = True) -> list[Path]:
        ...
        if dry_run and old_files:
            logger.info(
                f"Found {len(old_files)} files older than {days_old} days (dry run)"
            )
