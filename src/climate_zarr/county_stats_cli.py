...
def main():
    ...
    parser.add_argument("zarr_path", type=Path, help="Path to Zarr dataset")
    parser.add_argument("shapefile_path", type=Path, help="Path to county shapefile")
    parser.add_argument(
        "-o",
        "--output",
        type=Path,
    ...
    with ModernCountyProcessor(n_workers=args.workers) as processor:
        try:
            # Load shapefile
