...
class ChunkingConfig(BaseModel):
    ...
    auto_optimize: bool = Field(
        default=True, description="Auto-optimize chunks based on data"
    )

    def to_dict(self) -> Dict[str, int]:
        """Convert to dictionary for xarray."""
        return {"time": self.time, "lat": self.lat, "lon": self.lon}
...
class OutputConfig(BaseModel):
    ...
    base_output_dir: Path = Field(
        default=Path("./climate_outputs"), description="Base output directory"
    )
    create_subdirs: bool = Field(
        default=True, description="Create organized subdirectories"
    )
    include_timestamp: bool = Field(
        default=False, description="Include timestamp in directory names"
    )

    # File naming patterns
    naming_convention: str = Field(
        default="descriptive", description="File naming convention"
    )
    include_metadata: bool = Field(
        default=True, description="Include metadata in filenames"
    )
    date_format: str = Field(default="%Y%m%d", description="Date format for filenames")

    # File organization
    organize_by_variable: bool = Field(
        default=True, description="Organize files by climate variable"
    )
    organize_by_region: bool = Field(
        default=True, description="Organize files by region"
    )
    organize_by_scenario: bool = Field(
        default=True, description="Organize files by scenario"
    )
...
class ClimateConfig(BaseModel):
    ...
    regions: Dict[str, RegionConfig] = Field(
        default_factory=lambda: {
            "conus": RegionConfig(
                name="CONUS", lat_min=24.0, lat_max=50.0, lon_min=-125.0, lon_max=-66.0
            ),
            "alaska": RegionConfig(
                name="Alaska",
                lat_min=54.0,
            ...
            "guam": RegionConfig(
                name="Guam/MP", lat_min=13.0, lat_max=21.0, lon_min=144.0, lon_max=146.0
            ),
    ...
    prefer_virtual: bool = Field(
        default=True, description="Prefer virtual stores when possible"
    )
    kerchunk_fallback: bool = Field(
        default=True, description="Use Kerchunk if VirtualiZarr fails"
    )
    ...
    def get_region(self, name: str) -> RegionConfig:
        ...
        if name.lower() not in self.regions:
            raise ValueError(
                f"Unknown region: {name}. Available: {list(self.regions.keys())}"
            )
    ...
    def from_env(cls) -> "ClimateConfig":
        ...
        if naming_convention := os.getenv("CLIMATE_NAMING_CONVENTION"):
            config_data.setdefault("output", {})["naming_convention"] = (
                naming_convention
            )

        if create_subdirs := os.getenv("CLIMATE_CREATE_SUBDIRS"):
            config_data.setdefault("output", {})["create_subdirs"] = (
                create_subdirs.lower() == "true"
            )
