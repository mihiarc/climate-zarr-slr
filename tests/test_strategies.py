...
class TestVectorizedStrategy:
    ...
    def test_process_precipitation_data(
        ...
        with patch(
            "climate_zarr.utils.data_utils.calculate_precipitation_stats"
        ) as mock_stats:
    ...
    def test_process_temperature_data(self, sample_counties, sample_temperature_data):
        ...
        # Mock the stats function
        with patch(
            "climate_zarr.utils.data_utils.calculate_temperature_stats"
        ) as mock_stats:
            mock_stats.return_value = {
                "year": 2020,
                "scenario": "test",
    ...
    def test_process_with_invalid_variable(
        self, sample_counties, sample_precipitation_data
    ):
    ...
    def test_process_with_empty_counties(self, sample_precipitation_data):
        ...
        empty_counties = gpd.GeoDataFrame(
...
class TestStrategyDataTypes:
    ...
    def test_float32_data(self, sample_counties):
        ...
        float32_data = (
            xr.DataArray(
                np.random.random((len(time), len(lats), len(lons))).astype(np.float32)
                * 10,
                dims=["time", "y", "x"],
                coords={"time": time, "y": lats, "x": lons},
                name="pr",
