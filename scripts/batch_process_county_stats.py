...
class BatchCountyProcessor:
    ...
    def __init__(self, base_dir: Path = None, n_workers: int = 4, use_chunked_strategy: bool = True):
    ...
    def discover_datasets(self) -> List[Dict]:
        ...
        for variable in self.variables:
            for region in self.regions:
                zarr_path = (
                    self.zarr_dir / variable / region / self.scenario / 
        ...
        try:
            ...
            with ModernCountyProcessor(n_workers=self.n_workers) as processor:
                ...
                if strategy_name == 'SpatialChunked':
                    ...
                    console.print("[cyan]Strategy: Vectorized (sequential processing)[/cyan]")
                ...
                # Process the dataset
                console.print(f"[yellow]Processing {variable} data with {strategy_name} strategy...[/yellow]")
                
                # Show memory status
                mem_info = psutil.virtual_memory()
                console.print(f"[dim]Memory available: {mem_info.available/(1024**3):.1f}GB / {mem_info.total/(1024**3):.1f}GB[/dim]")
                ...
                if self.use_chunked_strategy:
                    ...
                    if memory_stats:
                        console.print(f"[dim]Peak memory usage: {memory_stats.get('used_gb', 0):.2f} GB[/dim]")
                ...
                if self.use_chunked_strategy and 'memory_stats' in locals() and memory_stats:
                ...
                processing_stats = {
                    ...
                    'counties_processed': len(results_df['county_id'].unique()) if 'county_id' in results_df.columns else 0,
                    'years_processed': len(results_df['year'].unique()) if 'year' in results_df.columns else 0,
                    'output_path': str(output_path),
                    'strategy_used': strategy_name,
                    'throughput_counties_per_minute': (len(results_df['county_id'].unique()) / processing_time * 60) if processing_time > 0 else 0
                }
                
                console.print(f"[green]✓ Successfully processed {len(results_df):,} records[/green]")
                console.print(f"[green]  Output: {output_path}[/green]")
                console.print(f"[dim]  Time: {processing_time:.1f}s, Memory: {memory_used:.2f} GB[/dim]")
                if 'throughput_counties_per_minute' in processing_stats:
                    console.print(f"[dim]  Throughput: {processing_stats['throughput_counties_per_minute']:.1f} counties/min[/dim]")
    ...
    def generate_summary_report(self, datasets: List[Dict]) -> None:
        ...
        if self.processing_log:
            total_time = sum(log['stats'].get('processing_time_seconds', 0) for log in self.processing_log if log['success'])
            total_records = sum(log['stats'].get('records_processed', 0) for log in self.processing_log if log['success'])
            avg_memory = np.mean([log['stats'].get('memory_used_gb', 0) for log in self.processing_log if log['success']])
            
            console.print(f"  Total processing time: {total_time:.1f} seconds ({total_time/60:.1f} minutes)")
        ...
        if self.processing_log:
            ...
            for log in self.processing_log:
                if log['success']:
                    ...
                    strategy_performance[strategy]['total_time'] += log['stats'].get('processing_time_seconds', 0)
            ...
            for strategy, count in strategy_counts.items():
                ...
                console.print(f"    - Avg throughput: {avg_throughput:.1f} counties/min")
...
Performance Expectations:
  ...
  - Throughput: 15-20+ counties/minute
    ...
    if args.memory_target != 0.75:
        ...
        console.print(f"[yellow]Memory target set to {args.memory_target*100:.0f}%[/yellow]")
