...
df = pd.read_csv(
    "climate_outputs/stats/pr/conus/ssp370/conus_ssp370_pr_stats_threshold25.4mm.csv"
)
...
fig.suptitle(
    "CONUS Precipitation Analysis by County (SSP370, 2015-2100 Average)",
    fontsize=16,
    fontweight="bold",
)
...
ax2 = axes[0, 1]
map_data.plot(
    column="days_above_threshold",
    ax=ax2,
    cmap=threshold_cmap,
    legend=True,
    legend_kwds={
        "label": "Days > 25.4mm (1 inch)",
        "orientation": "horizontal",
        "pad": 0.03,
        "fraction": 0.05,
    },
...
ax2.set_title(
    "Average Days with Heavy Rainfall (>1 inch)", fontsize=14, fontweight="bold"
)
ax2.axis("off")
...
map_data["precip_intensity"] = map_data["total_annual_precip_mm"] / (
    365 - df.groupby("county_id")["dry_days"].mean().values[0]
)
...
ax4.set_title(
    "Precipitation Intensity (mm per wet day)", fontsize=14, fontweight="bold"
)
ax4.axis("off")
...
map_data.plot(
    ...
    cmap=precip_cmap,
    legend=True,
    legend_kwds={"label": "Average Annual Precipitation (mm)", "shrink": 0.7},
    edgecolor="#666666",
    linewidth=0.2,
    missing_kwds={"color": "lightgrey"},
)
...
ax.set_title(
    "CONUS Average Annual Precipitation by County\n(SSP370 Scenario, 2015-2100)",
    fontsize=16,
    fontweight="bold",
    pad=20,
)
