...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("climate_transform.log"), logging.StreamHandler()],
)
...
class ClimateDataTransformer:
    ...
    def _load_format_spec(self) -> Dict:
        ...
        try:
            ...
            logger.error(
                f"Format specification file not found: {self.format_spec_path}"
            )
    ...
    def discover_files(self) -> Dict[str, List[Path]]:
        ...
        for variable in self.variable_mappings.keys():
            ...
            if files:
                ...
                console.print(
                    f"  ⚠ No {variable.upper()} files found (pattern: {pattern})"
                )
    ...
    def load_climate_data(
        ...
        with Progress(
            ...
            load_task = progress.add_task(
                "Loading climate data files", total=total_files
            )

            for variable, file_list in files.items():
                if not file_list:
                    console.print(
                        f"  [yellow]Skipping {variable.upper()} - no files found[/yellow]"
                    )
                    continue
                ...
                if dfs:
                    ...
                    console.print(
                        f"  ✓ Loaded {len(climate_data[variable])} records for {variable.upper()}"
                    )

                    # Log basic statistics
                    logger.info(
                        f"{variable.upper()} data shape: {climate_data[variable].shape}"
                    )
                    logger.info(
                        f"  Years: {climate_data[variable]['year'].min()} - {climate_data[variable]['year'].max()}"
                    )
                    logger.info(
                        f"  Unique counties: {climate_data[variable]['county_id'].nunique()}"
                    )
    ...
    def _calculate_daysabove90F(self, tasmax_data: pd.DataFrame) -> pd.DataFrame:
        ...
        if "days_above_35c" in tasmax_data.columns:
            ...
            logger.warning(
                "Using days_above_35c as proxy for days_above_90F (32.2°C). "
            ...
            if "threshold_temp_c" in tasmax_data.columns:
                ...
                if correct_threshold.any():
                    ...
                    tasmax_data["daysabove90F"] = tasmax_data["days_above_threshold_c"]
                    logger.warning(
                        f"No 32.2°C threshold found. Using existing threshold "
            ...
            logger.warning(
                "No threshold temperature data found in tasmax files. "
    ...
    def transform_data(self, climate_data: Dict[str, pd.DataFrame]) -> pd.DataFrame:
        ...
        with Progress(
            ...
            transform_task = progress.add_task(
                "Processing variables", total=len(climate_data)
            )
            ...
            for variable, df in climate_data.items():
                ...
                var_data["merge_key"] = (
                    var_data["county_id"].astype(str)
                    + "_"
                    + var_data["year"].astype(str)
                    + "_"
                    + var_data["region"].astype(str)
                    + "_"
                ...
                if before_dedup != after_dedup:
                    logger.warning(
                        f"  Removed {before_dedup - after_dedup} duplicate records from {variable.upper()}"
                    )
                ...
                logger.info(
                    f"  Processed {len(target_data)} records for {variable.upper()}"
                )
        ...
        logger.info(
            f"Using {primary_variable.upper()} as primary dataset ({variable_sizes[primary_variable]} records)"
        )
        ...
        for variable in variable_names:
            ...
            logger.info(
                f"  Merged {variable.upper()}: {before_merge} -> {len(merged_data)} records"
            )
    ...
    def _apply_final_transformations(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        result_df["cid2"] = pd.to_numeric(
            result_df["county_id"], errors="coerce"
        ).astype("Int64")
        ...
        if "county_name" in result_df.columns and "state" in result_df.columns:
            ...
            logger.warning(
                "County name or state column missing. Using county_id as name."
            )
        ...
        for col_spec in self.target_format["columns"]:
            ...
            if col_name not in result_df.columns:
                ...
                logger.warning(
                    f"Column '{col_name}' not found in data. Created with default values."
                )
        ...
        for col_spec in self.target_format["columns"]:
            ...
            if col_name in result_df.columns:
                try:
                    if col_type == "integer":
                        result_df[col_name] = pd.to_numeric(
                            result_df[col_name], errors="coerce"
                        ).astype("Int64")
                    elif col_type == "numeric":
                        result_df[col_name] = pd.to_numeric(
                            result_df[col_name], errors="coerce"
                    ...
                    logger.warning(
                        f"Error converting column '{col_name}' to {col_type}: {e}"
                    )
    ...
    def validate_output(self, df: pd.DataFrame) -> Dict[str, Union[int, float, List]]:
        ...
        table.add_row(
            "Year Range",
            f"{validation_results['year_range'][0]} - {validation_results['year_range'][1]}",
        )
        table.add_row(
            "Data Quality Issues", f"{len(validation_results['data_quality_issues'])}"
        )
        ...
        if validation_results["missing_data"]:
            ...
            for col, stats in validation_results["missing_data"].items():
                missing_table.add_row(
                    col, f"{stats['count']:,}", f"{stats['percentage']:.1f}%"
                )
    ...
    def save_results(
        ...
        metadata_path = (
            self.output_dir / f"{output_filename.replace('.csv', '_metadata.json')}"
        )
        import json
        ...
        report_path = (
            self.output_dir
            / f"{output_filename.replace('.csv', '_validation_report.txt')}"
        )
        with open(report_path, "w") as f:
            ...
            f.write(
                f"  Year Range: {validation_results['year_range'][0]} - {validation_results['year_range'][1]}\n\n"
            )
    ...
    def run_transformation(
        ...
        try:
            ...
            if total_files == 0:
                console.print(
                    "[bold red]❌ No climate statistics files found![/bold red]"
                )
            ...
            if separate_by_region:
                ...
                if "region" not in transformed_data.columns:
                    console.print(
                        "[bold red]❌ Region column not found. Cannot separate by region.[/bold red]"
                    )
                    console.print("Saving as single file instead...")
                    final_data = transformed_data.copy()
                    output_path = self.save_results(
                        final_data, validation_results, output_filename
                    )
                    return output_path
                ...
                for region in transformed_data["region"].unique():
                    ...
                    region_data = transformed_data[
                        transformed_data["region"] == region
                    ].copy()
                    ...
                    output_path = self.save_results(
                        region_data, region_validation, region_filename
                    )
                    output_paths.append(output_path)

                    console.print(
                        f"  ✓ Saved {region} data: {len(region_data)} records"
                    )
  ...
  python transform_climate_stats.py --output climate_data_2025.csv
    ...
    try:
        output_paths = transformer.run_transformation(
            output_filename=args.output, separate_by_region=args.separate_by_region
        )
