...
df = pd.read_csv(
    "climate_outputs/stats/tas/conus/ssp370/conus_ssp370_tas_stats_threshold0.0c.csv"
)
...
map_data.plot(
    column="mean_temp_fahrenheit",
    ax=ax,
    cmap=temp_cmap,
    legend=True,
    legend_kwds={"label": "Mean Annual Temperature (°F)", "shrink": 0.7},
    edgecolor="#666666",
    linewidth=0.2,
...
ax.set_title(
    "CONUS Average Annual Temperature by County\n(SSP370 Scenario, 2015-2100)",
    fontsize=16,
    fontweight="bold",
    pad=20,
)
...
map_data.plot(
    ...
    cmap="YlOrRd",
    legend=True,
    legend_kwds={"label": "Annual Temperature Range (°C)", "shrink": 0.7},
    edgecolor="#666666",
    linewidth=0.2,
    missing_kwds={"color": "lightgrey"},
)
...
ax2.set_title(
    "CONUS Annual Temperature Range by County\n(SSP370 Scenario, 2015-2100 Average)",
    fontsize=16,
    fontweight="bold",
    pad=20,
)
...
print("\n=== Temperature Summary ===")
print(
    f"Average Temperature: {county_avg['mean_annual_temp_c'].mean():.2f}°C ({county_avg['mean_temp_fahrenheit'].mean():.2f}°F)"
)
print(
    f"Coldest County: {county_avg.loc[county_avg['mean_annual_temp_c'].idxmin(), 'county_name']}, {county_avg.loc[county_avg['mean_annual_temp_c'].idxmin(), 'state']}"
)
print(
    f"  Temperature: {county_avg['mean_annual_temp_c'].min():.2f}°C ({county_avg['mean_temp_fahrenheit'].min():.2f}°F)"
)
print(
    f"Warmest County: {county_avg.loc[county_avg['mean_annual_temp_c'].idxmax(), 'county_name']}, {county_avg.loc[county_avg['mean_annual_temp_c'].idxmax(), 'state']}"
)
print(
    f"  Temperature: {county_avg['mean_annual_temp_c'].max():.2f}°C ({county_avg['mean_temp_fahrenheit'].max():.2f}°F)"
)
